...
const SAT_DEFS = [
  ...
  { id:3, name:'CHANDRAYAAN', r:7.9,  inc:0.93, raan:2.09, speed:0.44, phase:0.5 },
  { id:4, name:'STARLINK-4', r:7.9,  inc:0.93, raan:2.09, speed:0.44, phase:3.3 },
  { id:5, name:'PRITHVIYAAN',r:8.5,  inc:1.72, raan:1.75, speed:0.40, phase:0.8 },
  ...
  { id:9, name:'MOON',   r:7.6,  inc:0.72, raan:0.35, speed:0.46, phase:1.8 },
];
...
function initSatelliteScene(container) {
  ...
  const glowLight = new THREE.PointLight(0x88ccff, 1.5, 100);
...
function _buildEarth() {
  ...
  // 🌍 Earth texture (IMPORTANT: keep path correct)
  const earthMap = loader.load('earth_map.jpg');
  const cloudTexture = loader.load('earth_clouds.png');

  // ☁️ Clouds layer
  const clouds = new THREE.Mesh(
    new THREE.SphereGeometry(EARTH_R * 1.01, 64, 64),
  ...
  // store globally (needed for rotation)
...
function quickSelect(id) {
  hasFrozenOnce = false;   // 🔥 reset freeze for new selection
...
function animate() {
  ...
  // 🌥 clouds rotation always
  if (window.clouds) {
    window.clouds.rotation.y += 0.003;
  }
  ...
  if (!isFrozen) {
    ...
    SAT_DEFS.forEach(def=>{
      ...
      if(satMeshes[def.id])
        satMeshes[def.id].position.set(pos.x,pos.y,pos.z);
    });
    ...
    if(selectedSrc!==null && selectedDst!==null){
      ...
      if(newRoute.join(',')!==currentRoute.join(',')){
        ...
        // ✅ FREEZE LOGIC
        if (currentRoute.length > 1 && !hasFrozenOnce) {
          isFrozen = true;
          hasFrozenOnce = true;
          freezeEndTime = performance.now() + 15000;

          const hint=document.getElementById('sat-pick-hint');
          if(hint) hint.textContent = "🛰️ Path locked for 15s";
        }
        ...
        if(hint) hint.textContent=currentRoute.length>1
          ?`[${algoName}] Route: ${currentRoute.length-1} hops`
          :'No route';
      }
    }
