...
// Seal pending block every 10s — matches frontend recalc interval
setInterval(() => { if (ledger.pending.length > 0) ledger.sealBlock(); }, 10000);
...
app.post('/api/traffic/diff', (req,res) => {
  ...
  // Seal any pending blockchain transactions on every diff call
  // This ensures blocks appear in sync with the 10s recalc cycle
  if (ledger.pending.length > 0) ledger.sealBlock();

  if (!src || !dst || !currentPaths || !currentPaths.length) {
    takeWeightSnapshot();
    return res.json({ changedEdges:changed, needsRecompute:false, paths:null,
      blockchain:{ totalBlocks:ledger.chain.length, isValid:ledger.isValid(), latestBlock:ledger.chain[ledger.chain.length-1] } });
  }
